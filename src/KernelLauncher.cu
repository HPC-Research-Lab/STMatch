...
namespace libra {
  ...
  __global__ void _parallel_match(Graph* dev_graph, Pattern* dev_pattern, CallStack* dev_callstack, JobQueue* job_queue) {
    ...
    __shared__ PatternMatcher pm[NWARPS_PER_BLOCK];
    ...
    if (threadIdx.x % WARP_SIZE == 0) {
      pm[local_wid].pattern = &pat;
      pm[local_wid].graph = &graph;
      pm[local_wid].stk = dev_callstack[global_wid];
      pm[local_wid].job_queue = job_queue;
    }
    __syncwarp();
