...
__device__
bool trans_layer(CallStack& _target_stk, CallStack& _cur_stk, Pattern* _pat, int _k){
    if(_target_stk.level<=_k) return false;
...
__device__
int trans_skt(CallStack* _all_stk, CallStack* _cur_stk, Pattern* pat, int* _mutex){
        //int local_mutex = 0;
        int max_left_task = 0;
        int stk_idx = -1;
        int at_level = -1;
        ...
        if(stk_idx!=-1){
            ...
            if(trans_layer(_all_stk[stk_idx], *_cur_stk, pat, at_level)){
                res =  stk_idx;
            }
