  ...
  for(vidType v0 = 0; v0 < vertices; v0++) {
    ...
    for(vidType idx1 = 0; idx1 < y0f0.size(); idx1++) {
      ...
      for(vidType idx2 = 0; idx2 < y0y1.size(); idx2++) {
        ...
        VertexSet y2 = g.N(v2);
        VertexSet y0y1y2f2 = intersection_set(y0y1, y2, v2);
        ...
        for(vidType idx3 = 0; idx3 < y0y1y2f2.size(); idx3++) {
          ...
          VertexSet y3 = g.N(v3);
          VertexSet y0y1n2f2y3 = intersection_set(y0y1n2f2, y3);
          VertexSet y0y1y2f2n3f3 = difference_set(y0y1y2f2, y3, v3);
          VertexSet n0n1y2y3 = intersection_set(n0n1y2, y3);
          for(vidType idx4 = 0; idx4 < y0y1y2f2n3f3.size(); idx4++) {
            vidType v4 = y0y1y2f2n3f3.begin()[idx4];
            VertexSet y4 = g.N(v4);
            VertexSet y0y1n2y3y4f2 = intersection_set(y0y1n2f2y3, y4, v2);
            VertexSet n0n1y2y3y4 = intersection_set(n0n1y2y3, y4);
            for(vidType idx5 = 0; idx5 < y0y1n2y3y4f2.size(); idx5++) {
              vidType v5 = y0y1n2y3y4f2.begin()[idx5];
              VertexSet y5 = g.N(v5);
              counter[0] += intersection_num(n0n1y2y3y4, y5);
            }
          }
        }
      }
...
const uint64_t data_complexity = 10373.4;
const uint64_t time_complexity = 13123.5;
