  ...
  for(vidType v0 = 0; v0 < vertices; v0++) {
    ...
    for(vidType idx1 = 0; idx1 < y0f0.size(); idx1++) {
      ...
      for(vidType idx2 = 0; idx2 < y0y1.size(); idx2++) {
        ...
        for(vidType idx3 = 0; idx3 < n0y1y2.size(); idx3++) {
          vidType v3 = n0y1y2.begin()[idx3];
          VertexSet y3 = g.N(v3);
          counter[0] += difference_num(y0n1y2, y3);
        }
      }
    }
  }
}
const uint64_t data_complexity = 34940.6;
const uint64_t time_complexity = 38121.9;
