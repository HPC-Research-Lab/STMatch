const int n_counters = 1;
std::vector<std::vector<uint64_t>> global_counters(omp_get_max_threads());
const vidType vertices = g.V();
#pragma omp parallel
{
  std::vector<uint64_t> &counter = global_counters.at(omp_get_thread_num());
  counter.resize(n_counters, 0);
        #pragma omp for schedule(dynamic,64) nowait
    for(vidType v0 = 0; v0 < vertices;++v0)    {
      VertexSet y0 = g.N(v0);
      VertexSet y0f0 = g.N(v0);
      for(vidType v1:y0f0)      {
        VertexSet y1 = g.N(v1);
        VertexSet y0y1 = intersection_set(y0, y1);
        for(vidType v2:y0y1)        {
          VertexSet y2 = g.N(v2);
          VertexSet y0y1n2 = difference_set(y0y1, y2);
          VertexSet y0y1y2f2 = intersection_set(y0y1, y2, v2);
          for(vidType v3:y0y1y2f2)          {
            VertexSet y3 = g.N(v3);
            VertexSet y0y1n2n3 = difference_set(y0y1n2, y3);
            for(vidType v4:y0y1n2n3)            {
              VertexSet y4 = g.N(v4);
              counter[0] += difference_num(y0y1n2n3, y4, v4);
            }
          }
        }
      }
    }
}//217
const uint64_t data_complexity = 1.45844e+07;
const uint64_t time_complexity = 1.10437e+07;
