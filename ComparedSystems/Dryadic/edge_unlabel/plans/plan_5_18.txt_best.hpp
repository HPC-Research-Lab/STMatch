const int n_counters = 1;
std::vector<std::vector<uint64_t>> global_counters(omp_get_max_threads());
const vidType vertices = g.V();
#pragma omp parallel
{
  std::vector<uint64_t> &counter = global_counters.at(omp_get_thread_num());
  counter.resize(n_counters, 0);
        #pragma omp for schedule(dynamic,64) nowait
    for(vidType v0 = 0; v0 < vertices;++v0)    {
      VertexSet y0 = g.N(v0);
      for(vidType v1:y0)      {
        VertexSet y1 = g.N(v1);
        VertexSet y0n1f1 = difference_set(y0, y1, v1);
        VertexSet y0y1f1 = intersection_set(y0, y1, v1);
        for(vidType v2:y0y1f1)        {
          VertexSet y2 = g.N(v2);
          VertexSet y0n1f1y2 = intersection_set(y0n1f1, y2);
          VertexSet y0y1f1n2f2 = difference_set(y0y1f1, y2, v2);
          for(vidType v3:y0y1f1n2f2)          {
            VertexSet y3 = g.N(v3);
            counter[0] += intersection_num(y0n1f1y2, y3, v1);
          }
        }
      }
    }
}//217
const uint64_t data_complexity = 5.63195e+07;
const uint64_t time_complexity = 6.3768e+06;
