  ...
  counter.resize(n_counters, 0);
    ...
    for(vidType v0 = 0; v0 < vertices;++v0)    {
      ...
      for(vidType v1:y0f0)      {
        ...
        for(vidType v2:y0f0n1f1)        {
          VertexSet y2 = g.N(v2);
          VertexSet n0y1y2f0 = intersection_set(n0f0y1, y2, v0);
          VertexSet y0y1y2 = intersection_set(y0y1, y2);
          for(vidType v3:n0y1y2f0)          {
            VertexSet y3 = g.N(v3);
            VertexSet y0y1y2y3 = intersection_set(y0y1y2, y3);
            for(vidType v4:y0y1y2y3)            {
              VertexSet y4 = g.N(v4);
              VertexSet n0n1n2n3y4; counter[0] += difference_num(difference_set(n0n1n2n3y4,difference_set(n0n1n2n3y4,difference_set(n0n1n2n3y4,y4, y0), y1), y2), y3);
            }
          }
        }
      }
...
const uint64_t data_complexity = 4.68923e+07;
const uint64_t time_complexity = 3.09167e+07;
