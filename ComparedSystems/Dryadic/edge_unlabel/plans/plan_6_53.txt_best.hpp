    ...
    for(vidType v0 = 0; v0 < vertices;++v0)    {
      ...
      for(vidType v1:y0f0)      {
        ...
        for(vidType v2:y0y1)        {
          ...
          for(vidType v3:y0y1n2)          {
            VertexSet y3 = g.N(v3);
            VertexSet y0y1n2y3 = intersection_set(y0y1n2, y3);
            for(vidType v4:y0y1n2y3)            {
              VertexSet y4 = g.N(v4);
...
const uint64_t data_complexity = 1.45896e+07;
const uint64_t time_complexity = 1.10448e+07;
