...
RestSet::RestSet(const std::vector<int>& i, const std::vector<int>& o, const std::vector<int>& restrictions, const int& l) : ins(i), out(o),label(l) {
  depth =0;
...
void RestSet::append_calc_to_stream(std::ostream& oss,int index,std::set<RestSet> &available) const{
  ...
  //starting set, l0
  if(ins.size()==0&&out.size()==0){
    if(label!=-1)
      oss<< "VertexSet l"<<label<<" = g.L("<<label<<");\n";
    //if this is empty it's just iterating over everything anyway
    return;
  }
  //just a starting set, y0/y6l0
  if(ins.size()==1&&out.size()==0){
    oss << "VertexSet " << varname << " = g.N(v" << ins[0];
  ...
  if(par.ins.size() == 0) { // has transient unnamed intermediate sets
	...
	if(label!=-1)oss<<","<<label;
	oss<<"); ";
        oss<< "VertexSet " << varname << " = bounded(y0";
	if(label!=-1)oss<<"l"<<label;
	oss<<",v0);\n";
      }
      else {
