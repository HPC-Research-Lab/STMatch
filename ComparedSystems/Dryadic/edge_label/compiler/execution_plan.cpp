...
ExecutionPlan::ExecutionPlan(DirectedGraphlet &g) : vertices(g.vertices), restrictions(g.vertices,-1), depend(g.vertices) ,labelled(false),labels(g.vertices,-1) {
...
ExecutionPlan::ExecutionPlan(DirectedGraphlet &g, std::vector<int> rest) : vertices(g.vertices), restrictions(rest),depend(g.vertices) ,labelled(false),labels(g.vertices,-1){
  for(auto t : g.edgelist()) {
    int src, dst;
    bool exists;
    std::tie(src, dst, exists) = t;
...
ExecutionPlan::ExecutionPlan(DirectedGraphlet &g, std::vector<int> rest,std::vector<int> lab) : vertices(g.vertices), restrictions(rest),depend(g.vertices) ,labelled(true),labels(lab){
  for(auto t : g.edgelist()) {
    int src, dst;
    bool exists;
    std::tie(src, dst, exists) = t;
