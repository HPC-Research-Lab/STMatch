...
void MultiRed::reduced(std::vector<int> &ord,std::vector<int> &restrictions){
  ...
  ExecutionPlan ex(dg,applied_restrictions,labels);
  //print ordering and restrictions for now
  plans->push_back(ex);
  if(iterplans){
    for(int i=0;i<n;++i){
      //remove unset
      if(applied_restrictions[i]<2)applied_restrictions[i]=-1;
    }
    ExecutionPlan exi(dg,applied_restrictions,labels);
...
void MultiRed::autos(std::vector<int> &partial,
  ...
  for(int x : remaining){
    ...
    if(g.adjacency.at(x).size()!=g.adjacency.at(curr).size()
       ||g.labels[x]!=g.labels[curr])continue;
...
MultiRed::MultiRed(Graphlet& d, std::vector<ExecutionPlan>& p): g(d),n(d.vertices),plans(&p),iterplans(0){
...
MultiRed::MultiRed(Graphlet& d, std::vector<ExecutionPlan>& p,std::vector<std::vector<ExecutionPlan>>& ip): g(d),n(d.vertices),plans(&p),iterplans(&ip){
  //make the automorphisms
  std::set<int> autorem;
  for(int i=0;i<n;++i)autorem.insert(i);
  std::vector<int> init;
