...
public:
  ...
  ExecutionPlan(DirectedGraphlet&,std::vector<int>,std::vector<int>);
  std::string toString() const;
...
public:
  ...
  MultiRed(Graphlet&,std::vector<ExecutionPlan> &,std::vector<std::vector<ExecutionPlan>>&);
...
public:
  ...
  RestSet(const std::vector<int>&, const std::vector<int>&, const std::vector<int>&,const int& l=-1);
...
public:
  ...
  std::pair<std::string, std::string> to_code(int,int,int);
  double data_complexity();
  double time_complexity();
private:
